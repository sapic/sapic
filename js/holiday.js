...
CAnimation = function(rgAnimation, $Parent, x, y) {
   this.m_rgAnimation = $.extend({}, g_rgAnimationDefaults, rgAnimation);
   this.m_x = x;
   this.m_y = y;

   this.m_$Element = $('<div/>', {
      'class': 'holidayprofile_animation'
   });
...
CAnimation.prototype.Start = function() {
   this.m_start = $.now();
   this.m_interval = window.setInterval($.proxy(this.Tick, this), ANIMATION_TICK_RATE);
   CAnimation.sm_cAnimationsRunning++;
};

CAnimation.prototype.Tick = function() {
   var sElapsed = $.now() - this.m_start;
...
function StartAnimation() {
   var $Showcases = $('.profile_customization:not(.none_selected)');
   if (!$Showcases.length)
      return;

   AnimationForShowcase($($Showcases[0]));

   $Showcases.click(function() {
      AnimationForShowcase($(this));
   });
   ...
   animation = window.setInterval(function() {
      if (CAnimation.sm_cAnimationsRunning == 0 && Math.random() < 0.25) {
         ...
         if ($VisibleShowcases.length) {
            ...
            AnimationForShowcase($($VisibleShowcases[nShowcase]));
         }
      }
   }, 1500);
}
